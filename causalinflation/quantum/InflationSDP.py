...
class InflationSDP(object):
    ...
    def set_values(self,
        ...
        use_lpi_constraints : bool, optional
            ...
            <http://www.arxiv.org/abs/2203.16543/>`_) will be imposed or not. By
            default ``False``.
            ...
            for var, monomial_factors in self.semiknowable_atoms:
                ...
                if all(numeric_factors <= 1.):
                    ...
                    if np.sum(numeric_factors > 1) == 1:
                        ...
                        self.semiknown_moments[var] = \
                            [sorted_factors[:-1].prod(), sorted_factors[-1]]
    ...
    def solve(self,
        ...
        solveSDP_arguments = {"positionsmatrix":  self.momentmatrix,
                              ...
                              "var_equalities":   self.moment_equalities,
                              "var_inequalities": self.moment_inequalities,
                              "solve_dual":       dualise}
    ...
    def certificate_as_correlators(self,
                                   ...
                                   use_langlerangle: bool=False) -> sp.core.symbol.Symbol:
        ...
        CONSTANT_MOMENT = 1
        polynomial = dual[CONSTANT_MOMENT]
        for var, coeff in dual.items():
            if var != CONSTANT_MOMENT:
                monomial = 1
                for factor in vars_to_factors[var]:
                    factor_asnumbers = to_numbers(vars_to_names[factor], self.names)
                    parties, inputs = [], []
        ...
        # Clean coefficients here
        if clean:
            coeff_dict = polynomial.as_coefficients_dict()
            longest = sorted(list(coeff_dict.keys()), key=lambda x: len(str(x)))[-1]
            renorm = coeff_dict[longest]

            float_subs = {}
            for n in polynomial.atoms(sp.Float):
                if abs(n) > chop_tol:
                    # If after renomarlising, we have an order of magnitude
        ...
        if type(column_specification) == list:
            ...
            if type(column_specification[0]) in [list, np.ndarray]:
                if len(np.array(column_specification[1]).shape) == 2:
                    ...
                    columns = [np.array(mon, dtype=np.uint8)
                ...
                # If list of symbols, convert them to numeric form
                columns = []
                for col in column_specification:
                    if col == sp.S.One or col == 1:
                        columns += [np.array([0], dtype=np.uint8)]
                    else:
                        columns += [np.array(to_numbers(str(col), self.names),
                                             dtype=np.uint8)]
    ...
    def _calculate_inflation_symmetries(self) -> List[List[int]]:
        """Calculates the permutations of the generating columns derived from
        ...
        for source, permutation in tqdm(sorted(
                [(source, permutation) for source in range(n_sources)
                    for permutation in itertools.permutations(
                                                         range(inflevel[source])
                                                              )]
                                               ),
                                        disable=not self.verbose,
    ...
    def _factorize_monomials(self,
        ...
        Tuple[numpy.ndarray, numpy.ndarray]
            The factorised monomials reordered according to knowable,
            semi-knowable and unknowable moments and also the corresponding
            unfactorised monomials.
        """
        monomials_factors = factorize_monomials(
                                monomialset_name2num(monomials, self.names),
                                                verbose=self.verbose)
        monomials_factors_names = monomialset_num2name(monomials_factors,
                                                       self.names)
        ...
        monomials_unfactorised_reordered = \
            monomials_unfactorised_reordered.astype(object)
        monomials_unfactorised_reordered[:, 0] = \
            monomials_unfactorised_reordered[:, 0].astype(int)

        return monomials_factors_reordered, monomials_unfactorised_reordered

    def _find_positive_monomials(self,
                                 monomials_factors: np.ndarray,
                                 sandwich_positivity=True) -> np.ndarray:
        """Out of the list of monomials provided as input, determine which of
        them have always non-negative expectation values. This is the case if
        for every party there are no overlaps in inflation indices.
