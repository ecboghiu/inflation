...
Evans = InflationProblem({"U_AB": ["A", "B"],
                         ...
                         inflation_level_per_source=(1, 1),  # TO BE MODIFIED
                         order=("A", "B", "C"))
...
bounds["P(C=1 do B##=0) >= "] = Evans_Unpacked.objective_value
...
Evans_Unpacked.solve()
bounds["P(C=1 do B##=1) >= "] = Evans_Unpacked.objective_value
...
bounds["P(C=1 do B##=0) >= "] = Evans_Interrupted.objective_value
...
Evans_Interrupted.solve()
bounds["P(C=1 do B##=1) >= "] = Evans_Interrupted.objective_value
