...
class InflationProblem(object):
    ...
    def __init__(self,  dag=[], outcomes_per_party=[], settings_per_party=[],
        ...
        if not dag:
            ...
            self.hypergraph = np.array([[1 for _ in range(self.nr_parties)]], dtype=np.uint8)
        else:
            self.hypergraph, self.graph_equalities = self._dag_to_hypergraph(dag)
    ...
    def _dag_to_hypergraph(self, dag):
        ...
        if is_network:
            ...
            party_position = {party: position for position, party in enumerate(parties)}
            ...
            raise Exception('Non-network scenarios are not implemented yet')
